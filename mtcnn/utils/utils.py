...
def draw_bboxes(self,img,rectangles,points):
    overlay = img.copy()
    for rectangle in rectangles:
        # cv2.putText(img, str(round(rectangle[4],2)),
        #             (int(rectangle[0]), int(rectangle[1])),
        #             cv2.FONT_HERSHEY_SIMPLEX,
        #             0.7, (0, 255, 0))
        cv2.rectangle(overlay, (int(rectangle[0]), int(rectangle[1])),
...
def filter_bboxes(bboxes,min_height=30,min_width=30):
        res = []
        for box in bboxes:
            x1,y1,x2,y2,_ = box
            if(x1>0 and x2>0 and y1>0 and y2>0 and x2-x1>min_width and y2-y1>min_height):
                res.append(box)
        return np.array(res)